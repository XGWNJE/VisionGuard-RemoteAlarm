...
if $FULL; then
    echo "[3/5] 同步 package.json + npm install ..."
    scp "$SCRIPT_DIR/package.json" "$VPS_HOST:${VPS_PATH}/package.json"
    ssh "$VPS_HOST" "cd ${VPS_PATH} && npm install --omit=dev"
    echo "  OK"
else
    echo "[3/5] 跳过 (仅 --full 模式同步依赖)"
fi
...
echo ""
REMOTE_VER=$(ssh "$VPS_HOST" "node -e \"console.log(require('${VPS_PATH}/package.json').version)\"")
